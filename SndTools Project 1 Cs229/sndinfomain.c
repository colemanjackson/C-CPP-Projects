	...
	if (argv[1] == NULL){
	   fileType = fileNameParser(stdin); 
		if (fileType == 1) {
			...
			keyValueFinder229(stdin, soundVar);
		...
		if (fileType == 2) {
			...
			format[3] = 'F';
			format[4] = '\0';
			keyValueFinderAIFF(stdin, soundVar);
		}

		else if (fileType == -1) {
			fprintf(stderr,
					"FATAL ERROR, Exiting Program. No results will be printed.\n");
			return 1;
...
//print the outputs
	...
	//	sprintf(input, "%s", argv[i]);
		...
		printf("   Duration: %01.0f:%02.0f:%05.02f\n", soundVar->hours,
				soundVar->minutes, soundVar->seconds);
	...
	if (argv[1]!= NULL && strcmp(argv[1], "-1") == 0) {	  //checks for the -1 switch
		...
		if (cs229File == NULL) { /*Null Check*/
			fprintf(stderr,
					"Null File, no results will be printed. Exiting Program.\n");
			return 1;
	...
	if (argv[1]!= NULL && strcmp(argv[1], "-h") == 0) {  // checks for the -h switch
		...
		fprintf(stderr,
				"This program is used to display information about a sound file, either in the AIFF or CS229 formats. \n ");
		fprintf(stderr,
				"This prgram supports passing of files and switches in the following format: sndinfo [switches] [file] [file] \n");
		fprintf(stderr, "The following switches are supported: \n");
		fprintf(stderr,
				"-h: will display the help message you are now reading. :)\n");
		fprintf(stderr,
				"-1: will not use the files passed into the function, and instead will prompt users for the path to the file to be read.\n");
		fprintf(stderr,
				"if no switches are specified, the program will read the files passed into the fuction and display the information about each file until all files are read. \n");
		fprintf(stderr,
				"if no files and no switches are specified, sndinfo reads from stdin.\n");
		fprintf(stderr, "\n");
		return 0;
	...
	for (i = 1; i < argc; i++) {
		...
		if (cs229File == NULL) {
			printf(
					"------------------------------------------------------------\n");
			fprintf(stderr, "Error in reading file @ argument index: %d\n", i);
			if (i == (argc - 1)) {
			printf(
					"------------------------------------------------------------\n");
		}
